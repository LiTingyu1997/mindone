...
def add_special_tokens(hidden_states, attention_mask, sos_token, eos_token):
    ...
    if attention_mask is not None:
        ...
        attention_mask = ops.concat([new_attn_mask_step, attention_mask, new_attn_mask_step], axis=-1)
    ...
    hidden_states = ops.concat([sos_token, hidden_states, eos_token], axis=1)
...
class AudioLDM2ProjectionModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.sos_embed = Parameter(ops.ones(langauge_model_dim))
        self.eos_embed = Parameter(ops.ones(langauge_model_dim))

        self.sos_embed_1 = Parameter(ops.ones(langauge_model_dim))
        self.eos_embed_1 = Parameter(ops.ones(langauge_model_dim))
        ...
        if self.use_learned_position_embedding is not None:
            self.learnable_positional_embedding = Parameter(
                ops.zeros((1, text_encoder_1_dim, max_seq_length))
            )

    def construct(
        self,
        hidden_states: Optional[Tensor] = None,
        hidden_states_1: Optional[Tensor] = None,
        attention_mask: Optional[Tensor] = None,
        ...
        hidden_states = ops.cat([hidden_states, hidden_states_1], axis=1)
        ...
        if attention_mask is not None and attention_mask_1 is not None:
            attention_mask = ops.cat([attention_mask, attention_mask_1], axis=-1)
        else:
            attention_mask = None
...
class AudioLDM2UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def construct(
        ...
        encoder_hidden_states_1: Optional[Tensor] = None,
        encoder_attention_mask_1: Optional[Tensor] = None,
        ...
        Args:
            ...
            encoder_hidden_states_1 (`Tensor`, *optional*):
            ...
            encoder_attention_mask_1 (`Tensor`, *optional*):
