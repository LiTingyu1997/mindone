...
def shift_spectrograms_right(
    input_values: ms.Tensor, reduction_factor: int = 1, attention_mask: Optional[ms.Tensor] = None
):
    """
...
class SpeechT5DecoderLayer(nn.Cell):
    ...
    def construct(
        ...
        """
        Args:
            hidden_states (`ms.Tensor`): input to the layer of shape `(batch, seq_len, hidden_size)`
...
class SpeechT5PreTrainedModel(MSPreTrainedModel):
    """
...
class SpeechT5Encoder(SpeechT5PreTrainedModel):
    ...
    def construct(
        ...
        """
        Args:
            hidden_states (`ms.Tensor` of shape `(batch_size, sequence_length, feature_size)`):
...
class SpeechT5GuidedMultiheadAttentionLoss(nn.Cell):
    ...
    def construct(
        ...
        Args:
            attentions (`ms.Tensor` of shape `(batch_size, layers * heads, output_sequence_length, input_sequence_length)`):
                Batch of multi-head attention weights
            input_masks (`ops.Tensor` of shape `(batch_size, input_sequence_length)`):
                Input attention mask as booleans.
            output_masks (`ops.Tensor` of shape `(batch_size, output_sequence_length)`):
                Target attention mask as booleans.

        Returns:
            `ms.Tensor` with the loss value
        """
        ...
        masks = masks.to(attentions).unsqueeze(1)
...
class SpeechT5SpectrogramLoss(nn.Cell):
    ...
    def __init__(self, config: SpeechT5Config):
        ...
        self.bce_criterion = BCEWithLogitsLoss(pos_weight=ms.Tensor(5.0))
    ...
    def construct(
        ...
        outputs_after_postnet: ms.Tensor,
        logits: ms.Tensor,
        labels: ms.Tensor,
        cross_attentions: Optional[ms.Tensor] = None,
    ) -> ms.Tensor:
        ...
        stop_labels = ops.cat([~masks * 1.0, ops.ones(masks.size(0), 1)], axis=1)
...
SPEECHT5_BASE_START_DOCSTRING = r"""
    ...
    This model is also a PyTorch [nn.Cell](https://pytorch.org/docs/stable/nn.html#nn.Cell) subclass.
...
SPEECHT5_INPUTS_DOCSTRING = r"""
    Args:
        ...
        past_key_values (`tuple(tuple(ms.Tensor))`, *optional*, returned when `use_cache=True` is passed or when `config.use_cache=True`):
            ...
            `decoder_input_values` of shape `(batch_size, sequence_length)`. decoder_inputs_embeds (`ms.Tensor`
...
class SpeechT5Model(SpeechT5PreTrainedModel):
    ...
    def construct(
        ...
        input_values: Optional[ms.Tensor] = None,
        attention_mask: Optional[ms.Tensor] = None,
        decoder_input_values: Optional[ms.Tensor] = None,
        ...
        decoder_input_values (`ms.Tensor` of shape `(batch_size, target_sequence_length)`, *optional*):
        ...
        speaker_embeddings (`ms.Tensor` of shape `(batch_size, config.speaker_embedding_dim)`, *optional*):
...
class SpeechT5ForSpeechToText(SpeechT5PreTrainedModel):
    ...
    def construct(
        ...
        input_values (`ms.Tensor` of shape `(batch_size, sequence_length)`):
            ...
            and conversion into a tensor of type `ms.Tensor`. See [`SpeechT5Processor.__call__`] for details.
...
def _generate_speech(
    ...
    while True:
        ...
        if idx < minlen:
            ...
            if idx < maxlen:
                meet_thresholds = ops.sum(prob, axis=-1) >= threshold
                meet_indexes = ops.where(meet_thresholds)[0].tolist()
            else:
...
class SpeechT5ForTextToSpeech(SpeechT5PreTrainedModel):
    ...
    def construct(
        ...
        speaker_embeddings: Optional[ms.Tensor] = None,
        labels: Optional[ms.Tensor] = None,
        stop_labels: Optional[ms.Tensor] = None,
        ...
        decoder_input_values (`ms.Tensor` of shape `(batch_size, sequence_length, config.num_mel_bins)`):
        ...
        speaker_embeddings (`ms.Tensor` of shape `(batch_size, config.speaker_embedding_dim)`, *optional*):
            Tensor containing the speaker embeddings.
        labels (`ms.Tensor` of shape `(batch_size, sequence_length, config.num_mel_bins)`, *optional*):
    ...
    def generate(
        ...
        **kwargs,
        ...
        Returns:
            `tuple(ms.Tensor)` comprising various elements depending on the inputs:
            - when `return_output_lengths` is False
                - **spectrogram** (*optional*, returned when no `vocoder` is provided) `ms.Tensor` of shape
                `(output_sequence_length, config.num_mel_bins)` -- The predicted log-mel spectrogram.
                - **waveform** (*optional*, returned when a `vocoder` is provided) `ms.Tensor` of shape
                `(num_frames,)` -- The predicted speech waveform.
                - **cross_attentions** (*optional*, returned when `output_cross_attentions` is `True`)
                `ms.Tensor` of shape `(config.decoder_layers, config.decoder_attention_heads,
            ...
            - when `return_output_lengths` is True
                - **spectrograms** (*optional*, returned when no `vocoder` is provided) `ms.Tensor` of shape
                ...
                - **waveforms** (*optional*, returned when a `vocoder` is provided) `ms.Tensor` of shape
                ...
                - **cross_attentions** (*optional*, returned when `output_cross_attentions` is `True`)
                `ms.Tensor` of shape `(batch_size, config.decoder_layers, config.decoder_attention_heads,
                output_sequence_length, input_sequence_length)` -- The outputs of the decoder's cross-attention layers.
        """
        if speaker_embeddings is not None:
    ...
    def generate_speech(
        ...
        Args:
            input_values (`ms.Tensor` of shape `(batch_size, sequence_length)`):
                ...
                of type `ms.Tensor`. See [`SpeechT5Processor.__call__`] for details.
            speaker_embeddings (`ms.Tensor` of shape `(batch_size, config.speaker_embedding_dim)`, *optional*):
                Tensor containing the speaker embeddings.
            attention_mask (`ms.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
                Mask to avoid performing convolution and attention on padding token indices. Mask values selected in
...
class SpeechT5HifiGan(MSPreTrainedModel):
