...
class HubertPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, (nn.LayerNorm, mint.nn.GroupNorm, nn.BatchNorm1d)):
            module.bias.data.zero_()
            module.weight.data.fill_(1.0)
        elif isinstance(module, mint.nn.Conv1d):
    ...
    def _get_feature_vector_attention_mask(self, feature_vector_length: int, attention_mask: mindspore.tensor):
        ...
        attention_mask[(mint.arange(attention_mask.shape[0], device=attention_mask.device), output_lengths - 1)] = 1
...
class HubertModel(HubertPreTrainedModel):
    def __init__(self, config: HubertConfig):
        ...
        if config.mask_time_prob > 0.0 or config.mask_feature_prob > 0.0:
            self.masked_spec_embed = nn.Parameter(mindspore.tensor(config.hidden_size).uniform_())
    ...
    def _mask_hidden_states(
        ...
        if mask_time_indices is not None:
            ...
            mask_time_indices = mindspore.tensor(mask_time_indices, device=hidden_states.device, dtype=mindspore.bool_)
            hidden_states[mask_time_indices] = self.masked_spec_embed.to(hidden_states.dtype)
        ...
        if self.config.mask_feature_prob > 0 and self.training:
            ...
            mask_feature_indices = mindspore.tensor(mask_feature_indices, device=hidden_states.device, dtype=mindspore.bool_)
...
class HubertForSequenceClassification(HubertPreTrainedModel):
    def __init__(self, config):
        ...
        if config.use_weighted_layer_sum:
            self.layer_weights = nn.Parameter(mint.ones(num_layers) / num_layers)
