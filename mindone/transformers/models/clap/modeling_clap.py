...
def interpolate(hidden_states, ratio):
    ...
    Args:
        hidden_states (`ms.Tensor` of shape (batch_size, time_length, classes_num)):
            Input hidden states
        ratio (`int`):
...
def window_partition(hidden_states, window_size):
    ...
    Args:
        hidden_states (`ms.Tensor` of shape `(batch_size, height, width, num_channels)`):
            Input hidden states
        window_size (`int`):
            Window size
    """
...
def window_reverse(windows, window_size, height, width):
    ...
    Args:
        windows (`ms.Tensor` of shape `(num_windows * batch_size, window_size, window_size, num_channels)`):
            Input windows
        window_size (`int`):
            Window size
        height (`int`):
...
class ClapAudioModelOutput(ModelOutput):
    ...
    Args:
        audio_embeds (`ms.Tensor` of shape `(batch_size, hidden_size)`):
    ...
    audio_embeds: Optional[ms.Tensor] = None
    last_hidden_state: ms.Tensor = None
    hidden_states: Optional[Tuple[ms.Tensor, ...]] = None
    attentions: Optional[Tuple[ms.Tensor, ...]] = None
...
class ClapOutput(ModelOutput):
    ...
    Args:
        ...
        logits_per_audio:(`ms.Tensor` of shape `(audio_batch_size, text_batch_size)`):
            The scaled dot product scores between `audio_embeds` and `text_embeds`. This represents the audio-text
            similarity scores.
        logits_per_text:(`ms.Tensor` of shape `(text_batch_size, audio_batch_size)`):
            The scaled dot product scores between `text_embeds` and `audio_embeds`. This represents the text-audio
            similarity scores.
        text_embeds(`ms.Tensor` of shape `(batch_size, output_dim`):
            The text embeddings obtained by applying the projection layer to the pooled output of [`ClapTextModel`].
        audio_embeds(`ms.Tensor` of shape `(batch_size, output_dim`):
    ...
    logits_per_audio: ms.Tensor = None
    logits_per_text: ms.Tensor = None
    text_embeds: ms.Tensor = None
    audio_embeds: ms.Tensor = None
    ...
    def construct(
        ...
        if self.position_embedding_type == "relative_key" or self.position_embedding_type == "relative_key_query":
            ...
            if self.position_embedding_type == "relative_key":
                relative_position_scores = ms.einsum("bhld,lrd->bhlr", query_layer, positional_embedding)
                attention_scores = attention_scores + relative_position_scores
            elif self.position_embedding_type == "relative_key_query":
                relative_position_scores_query = ms.einsum("bhld,lrd->bhlr", query_layer, positional_embedding)
                relative_position_scores_key = ms.einsum("bhrd,lrd->bhlr", key_layer, positional_embedding)
...
class ClapModel(ClapPreTrainedModel):
    ...
    def get_audio_features(
        ...
        r"""
        Returns:
            audio_features (`ms.Tensor` of shape `(batch_size, output_dim`): The audio embeddings obtained by
