...
class Wav2Vec2ForPreTrainingOutput(ModelOutput):
    ...
    Args:
        loss (*optional*, returned when `sample_negative_indices` are passed, `ms.Tensor` of shape `(1,)`):
            Total loss as the sum of the contrastive loss (L_m) and the diversity loss (L_d) as stated in the [official
            paper](https://arxiv.org/pdf/2006.11477.pdf) . (classification) loss.
        projected_states (`ms.Tensor` of shape `(batch_size, sequence_length, config.proj_codevector_dim)`):
            Hidden-states of the model projected to *config.proj_codevector_dim* that can be used to predict the masked
            projected quantized states.
        projected_quantized_states (`ms.Tensor` of shape `(batch_size, sequence_length, config.proj_codevector_dim)`):
        ...
        attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            heads.
        contrastive_loss (*optional*, returned when `sample_negative_indices` are passed, `ms.Tensor` of shape `(1,)`):
            The contrastive loss (L_m) as stated in the [official paper](https://arxiv.org/pdf/2006.11477.pdf) .
        diversity_loss (*optional*, returned when `sample_negative_indices` are passed, `ms.Tensor` of shape `(1,)`):
            The diversity loss (L_d) as stated in the [official paper](https://arxiv.org/pdf/2006.11477.pdf) .
    """

    loss: Optional[ms.Tensor] = None
    projected_states: ms.Tensor = None
    projected_quantized_states: ms.Tensor = None
    codevector_perplexity: ms.Tensor = None
    hidden_states: Optional[Tuple[ms.Tensor]] = None
    attentions: Optional[Tuple[ms.Tensor]] = None
    contrastive_loss: Optional[ms.Tensor] = None
    diversity_loss: Optional[ms.Tensor] = None
...
class Wav2Vec2GumbelVectorQuantizer(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.codevectors = Parameter(
            ms.Tensor(1, self.num_groups * self.num_vars, config.codevector_dim // self.num_groups)
        )
        self.weight_proj = mint.nn.Linear(config.conv_dim[-1], self.num_groups * self.num_vars)
    ...
    def _compute_perplexity(probs, mask=None):
        ...
        perplexity = mint.exp(-mint.sum(marginal_probs * mint.log(marginal_probs + 1e-7), dim=-1)).sum()
        return perplexity
    ...
    def construct(self, hidden_states, mask_time_indices=None):
        ...
        if self.training:
            ...
            codevector_probs = mint.nn.functional.gumbel_softmax(
                hidden_states.float(), tau=self.temperature, hard=True
            ).type_as(hidden_states)

            # compute perplexity
            codevector_soft_dist = mint.softmax(
...
class Wav2Vec2PreTrainedModel(MSPreTrainedModel):
    """
...
class Wav2Vec2ForPreTraining(Wav2Vec2PreTrainedModel):
    def __init__(self, config: Wav2Vec2Config):
        ...
        self.project_hid = mint.nn.Linear(config.hidden_size, config.proj_codevector_dim)
        self.project_q = mint.nn.Linear(config.codevector_dim, config.proj_codevector_dim)
    ...
    def compute_contrastive_logits(
        target_features: ms.Tensor,
        negative_features: ms.Tensor,
        predicted_features: ms.Tensor,
        temperature: int = 0.1,
    ):
        """
        ...
        """
        target_features = mint.cat([target_features, negative_features], dim=0)

        logits = mint.cosine_similarity(predicted_features.float(), target_features.float(), dim=-1).type_as(
            target_features
        )
    ...
    def construct(
        ...
        attention_mask: Optional[ms.Tensor] = None,
        mask_time_indices: Optional[ms.Tensor] = None,
        sampled_negative_indices: Optional[ms.Tensor] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple, Wav2Vec2ForPreTrainingOutput]:
        r"""
        mask_time_indices (`ms.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
            Indices to mask extracted features for contrastive loss. When in training mode, model learns to predict
            masked extracted features in *config.proj_codevector_dim* space.
        sampled_negative_indices (`ms.Tensor` of shape `(batch_size, sequence_length, num_negatives)`, *optional*):
        ...
        ... )
        >>> mask_time_indices = ms.tensor(data=mask_time_indices, dtype=ms.int64)
        >>> sampled_negative_indices = ms.tensor(
        ...     data=sampled_negative_indices, dtype=ms.int64
        ... )

        >>> outputs = model(input_values, mask_time_indices=mask_time_indices)

        >>> # compute cosine similarity between predicted (=projected_states) and target (=projected_quantized_states)
        >>> cosine_sim = mint.cosine_similarity(outputs.projected_states, outputs.projected_quantized_states, dim=-1)

        >>> # show that cosine similarity is much higher than random
        >>> cosine_sim[mask_time_indices.to(ms.bool_)].mean() > 0.5
        tensor(True)
        ...
        if mask_time_indices is not None:
            mask_time_indices = mask_time_indices.to(ms.bool_)
...
class AMSoftmaxLoss(nn.Cell):
    def __init__(self, input_dim, num_labels, scale=30.0, margin=0.4):
        ...
        self.weight = Parameter(mint.randn(input_dim, num_labels), requires_grad=True)
...
class TDNNLayer(nn.Cell):
    ...
    def construct(self, hidden_states: ms.Tensor) -> ms.Tensor:
        # for backward compatibility, we keep mint.nn.Linear but call F.conv1d for speed up
        hidden_states = hidden_states.transpose(1, 2)
        weight = self.kernel.weight.view(self.out_conv_dim, self.kernel_size, self.in_conv_dim).transpose(1, 2)
        hidden_states = mint.nn.functional.conv1d(hidden_states, weight, self.kernel.bias, dilation=self.dilation)
        hidden_states = hidden_states.transpose(1, 2)
